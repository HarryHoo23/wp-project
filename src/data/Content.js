...
const SHOWROOM = [
    {
        ...
        first_level: 0,
        upper_floor: 0,
        sale_price: "$2,289,600.00",
        lease_price: "$119,250.00",        
...
const WORKING_PLUS_STORAGE = [
    {
        unit_id: "e1",
        unit_number: "E1 (Under Offer)",
        ground_level: "110",        
        first_level: "55",
        upper_floor: 0,
        yard_space: "17",
        sale_price: "$1,140,000.00",
        lease_price: "$57,000.00",        
        allocated_car_spaces: 3,        
        sold: false,
        estimated_outgoings: {
            area: 165,
...
const addImageToUnits = (array, unitType) => {
    ...
    if (unitType === "office") {
        ...
        resultArray = array.map((element, index) => {
            return (
                { ...element, img: `${imageUrl}/c${index+1}-floorplan.png`, sitePlanImg: `${sitePlanImageUrl}/c${index+1}-internal-siteplan.png`, pdf: `${pdfUrl}WHITEFOX_Marketing_CVA_Viridian_Floorplan_OFFICE_${index + 1}.pdf`}
            )
        })
    }
    if (unitType === "hybrid") {
        ...
        let tempArray_A = array.slice(0,12).map((element, index) => {
            return (
                { ...element, img: `${imageUrl}/A/a${index+1}-floorplan.png`, sitePlanImg: `${sitePlanImageUrl}/A/a${index+1}-internal-siteplan.png`, pdf: `${pdfUrl}A/WHITEFOX_Marketing_CVA_Viridian_Floorplan_A${index + 1}.pdf` }
            )
        })
        let tempArray_D = array.slice(12,15).map((element, index) => {
            return (
                { ...element, img: `${imageUrl}/D/d${index+1}-floorplan.png`, sitePlanImg: `${sitePlanImageUrl}/D/d${index+1}-internal-siteplan.png`, pdf: `${pdfUrl}D/WHITEFOX_Marketing_CVA_Viridian_Floorplan_D${index + 1}.pdf` }
            )
        })
    ...
    if (unitType === "three") {
        ...
        resultArray = array.map((element, index) => {
            return (
                { ...element, img: `${imageUrl}/b${index+1}-floorplan.png`, sitePlanImg: `${sitePlanImageUrl}/b${index+1}-internal-siteplan.png`, pdf: `${pdfUrl}WHITEFOX_Marketing_CVA_Viridian_Floorplan_B${index + 1}.pdf` }
            )
        })
    }
    if (unitType === "storage") {
